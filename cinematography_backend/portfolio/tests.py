...
class PortfolioApiTests(TestCase):
    def setUp(self):
        ...
        self.hero_slide = HeroSlide.objects.create(
            image="test.jpg", video="test.mp4", title="Test Slide", category=self.category,
            views="10K", order=1, is_active=True
        )
    ...
    def test_hero_slide_list(self):
        response = self.client.get('/portfolio/hero-slides/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['title'], "Test Slide")
